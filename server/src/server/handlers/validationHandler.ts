...
import { CfnValidation } from "../../service/services/cfnValidation";
...
export class ValidationHandler extends YamlValidationHandler {
  ...
  private patchTemplateSchema(registrySchemaDirectory: string) {
    const stub = readFileSync(
      __dirname + "/../../schema/resource.schema.patch.json",
      "utf8"
    );
    let templateSchema = JSON.parse(
      readFileSync(__dirname + "/../../schema/resource.schema.json", "utf8")
    );
    ...
    writeFileSync(
      __dirname + "/../../schema/resource.schema.json",
  ...
  validateTextDocument(document: TextDocument): Promise<Diagnostic[]> {
    ...
    return new Promise<Diagnostic[]>((resolve, reject) => {
      if (template.isValidTemplate) {
        ...
        const cfnLint = new CfnValidation(this.cfnSettings.cfnLintPath, args);
